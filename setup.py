...
class InstallLocale(distutils.command.install_data.install_data):
    def run(self):
        ...
        for lang in os.listdir(os.path.join(__path__, "locale")):
            ...
            self.mkpath(path)
            self.copy_file(
                os.path.join(__path__, "locale", lang, "pygtkspellcheck.mo"),
...
distutils.command.install.install.sub_commands.append(
    ("install_locale", lambda self: True)
)
            ...
            (
                ...
                [
                    ...
                    for message_file in os.listdir(
                        os.path.join(windows_locale, lang, "LC_MESSAGES")
                    )
...
setup(
    ...
    extras_require={"building the documentation": ["sphinx"]},
    classifiers=[
